...
class Users extends CActiveRecord
{
    ...
    public function allUsuarios(){
	 $sql = "SELECT pn.PENA_ID, ROUND(p.PERS_IDENTIFICACION) AS USUARIO, ROUND(p.PERS_IDENTIFICACION) AS CLAVE
	 FROM TBL_PERSONAS p,TBL_PERSONASNATURALES pn, TBL_CONTRATOS c, TBL_OPSCONTRATOS oc, TBL_ANIOSACADEMICOS aa
	 WHERE pn.PERS_ID = p.PERS_ID AND p.PERS_ID = c.PERS_ID AND c.CONT_ID = oc.CONT_ID AND oc.ANAC_ID = aa.ANAC_ID 
	 AND aa.ANAC_ESTADO = 0 AND c.CONT_ID >=890 GROUP BY p.PERS_IDENTIFICACION ORDER BY USUARIO ASC";
	 $connection = Yii::app()->db;
	 $query = $connection->createCommand($sql)->queryAll();
	 return $query;
	}
	
	public function allUsuariosT(){
	 $sql = "SELECT pn.PENA_ID, ROUND(p.PERS_IDENTIFICACION) AS USUARIO, ROUND(p.PERS_IDENTIFICACION) AS CLAVE
	 FROM TBL_PERSONAS p,TBL_PERSONASNATURALES pn, TBL_CONTRATOS c, TBL_TUTORIASCONTRATOS tc, TBL_PERIODOSACADEMICOS pa
	 WHERE pn.PERS_ID = p.PERS_ID AND p.PERS_ID = c.PERS_ID AND c.CONT_ID = tc.CONT_ID AND tc.PEAC_ID = pa.PEAC_ID
	 AND pa.ANAC_ID = 2014  GROUP BY p.PERS_IDENTIFICACION ORDER BY USUARIO ASC";
	 $connection = Yii::app()->db;
	 $query = $connection->createCommand($sql)->queryAll();
	 return $query;
	}			
