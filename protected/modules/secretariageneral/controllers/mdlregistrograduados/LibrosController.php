...
class LibrosController extends Controller
{
	...
	{
		$model=new Libros;
		$libroActivo = $model->getLibroActivo();
		...
		if(isset($_POST['Libros']))
		{
			...
			$model->desactivarLibros();
			if($model->save()){
				$this->redirect(array('admin','id'=>$model->LIBR_ID));
		
		}else{
				  Yii::app()->user->setFlash('error','<strong>Oppss!. </strong>No se pueden tener dos libros activos.');
				 }
		}		
	...
	public function actionChangeState($id,$estado){
		$Libros = Libros::model()->findByPk($id);
		if($estado==0){$nuevoEstado = 1;}else{ $nuevoEstado = 0;}
		$cambiarEstado = $Libros->cambiarEstado($id,$nuevoEstado);
