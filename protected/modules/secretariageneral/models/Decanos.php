...
class Decanos extends CActiveRecord
{
  ...
  public function getDecanosFechaGrado($FECHA, $FACU_ID){
		
	  $connection = Yii::app()->db;
		  $sql="SELECT DECA_ID  FROM TBL_DECANOS WHERE '".$FECHA."' BETWEEN DECA_FECHA_INICIO AND DECA_FECHA_FIN AND FACU_ID=".$FACU_ID;
		   $dato=$connection->createCommand($sql)->queryScalar();
		return $dato;
		}

		
		public function getNombreDecanos($DECA_ID){
		
	  $connection = Yii::app()->db;
		  $sql="SELECT CONCAT(p.PENA_NOMBRES,' ',p.PENA_APELLIDOS)  FROM TBL_DECANOS r
		  INNER JOIN TBL_PERSONASNATURALES p ON  r.PENA_ID=p.PENA_ID
		   WHERE DECA_ID=".$DECA_ID."";
		   $dato=$connection->createCommand($sql)->queryScalar();
		return $dato;
		}
		
			public function getSexoDecano($DECA_ID){
		
	  $connection = Yii::app()->db;
		  $sql="SELECT S.SEXO_DESCRIPCION FROM TBL_DECANOS D
INNER JOIN TBL_PERSONASNATURALES P ON D.PENA_ID=P.PENA_ID
INNER JOIN TBL_SEXOS S ON P.SEXO_ID=S.SEXO_ID
 WHERE D.DECA_ID=".$DECA_ID."";
		   $dato=$connection->createCommand($sql)->queryScalar();
		return $dato;
		}
}