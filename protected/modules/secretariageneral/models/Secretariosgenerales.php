...
class Secretariosgenerales extends CActiveRecord
{
	...
	public function getImagenEstado(){
	  ...
	  public function getSecretarioporFechaGrado($FECHA){
		 $connection = Yii::app()->db;
		  $sql="SELECT SEGE_ID  FROM TBL_SECRETARIOSGENERALES WHERE '".$FECHA."' BETWEEN SEGE_FECHA_INICIO AND SEGE_FECHA_FIN";
		  $dato=$connection->createCommand($sql)->queryScalar();
	
	 return  $dato;
		
		}
		public function getNombreSecretarios($SEGE_ID){
		
	  $connection = Yii::app()->db;
		  $sql="SELECT CONCAT(p.PENA_NOMBRES,' ',p.PENA_APELLIDOS) FROM TBL_SECRETARIOSGENERALES r
		  INNER JOIN TBL_PERSONASNATURALES p ON  r.PENA_ID=p.PENA_ID
		   WHERE SEGE_ID=".$SEGE_ID."";
		   $dato=$connection->createCommand($sql)->queryScalar();
		return $dato;
		}
		
			public function getSexoSecretarios($SEGE_ID){
		
	  $connection = Yii::app()->db;
		  $sql="SELECT S.SEXO_DESCRIPCION FROM TBL_SECRETARIOSGENERALES D
INNER JOIN TBL_PERSONASNATURALES P ON D.PENA_ID=P.PENA_ID
INNER JOIN TBL_SEXOS S ON P.SEXO_ID=S.SEXO_ID
 WHERE D.SEGE_ID=".$SEGE_ID."";
		   $dato=$connection->createCommand($sql)->queryScalar();
		return $dato;
		}
}