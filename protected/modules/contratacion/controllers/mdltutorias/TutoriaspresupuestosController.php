...
class TutoriaspresupuestosController extends Controller
{
	/**
	...
	public function actionCreate()
	{
		...
		$Tutoriaspresupuestos = new Tutoriaspresupuestos;
		...
		if(isset($_POST['Presupuestos']))
		{
			...
			if($Presupuestos->save()){
				...
				$Tutoriaspresupuestos->PRES_ID = $lastData->PRES_ID;
				$Tutoriaspresupuestos->TUPR_FECHA_INGRESO = $lastData->PRES_FECHA_INGRESO; 
				if($Tutoriaspresupuestos->save()){
				 $this->redirect(array('admin','id'=>$Tutoriaspresupuestos->PRES_ID));
				 Yii::app()->user->setFlash('success','<strong>Hecho!. </strong>Exito actualizando el presupuesto...');
				}else{
			          Yii::app()->user->setFlash('error','<strong>Oppss!. </strong>Error creando el presupuesto - tutoria...');
			         }
			}else{
			       Yii::app()->user->setFlash('error','<strong>Oppss!. </strong>Error creando el presupuesto...');
			     }
		}
		$this->render('create',array('model'=>$Presupuestos,'Tutoriaspresupuestos'=>$Tutoriaspresupuestos));
	}
	...
	{
		$Presupuestos = Presupuestos::model()->findByPk($id); 		
		
		$Tutoriaspresupuestos = new Tutoriaspresupuestos;
		$Tutoriaspresupuestos = $this->loadModel($id);
		...
		$this->render('update',array(
			'model'=>$Presupuestos,
			'Tutoriaspresupuestos'=>$Tutoriaspresupuestos,
		));
	}
	...
	{
		$dataProvider=new CActiveDataProvider('Tutoriaspresupuestos');
	...
	{
		$model=new Tutoriaspresupuestos('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Tutoriaspresupuestos']))
			$model->attributes=$_GET['Tutoriaspresupuestos'];
	...
	{
		$model=Tutoriaspresupuestos::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='tutoriaspresupuestos-form')
		{
