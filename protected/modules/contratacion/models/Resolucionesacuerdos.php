...
class Resolucionesacuerdos extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('REAC_NUMERO, REAC_FECHA', 'required'),
	...
	public function relations()
	{
		...
		return array(
		'rel_contratantes' => array(self::HAS_ONE, 'Contratantes', 'REAC_ID'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			'REAC_ID' => 'Reac',
			'REAC_NUMERO' => 'NÚM. RESOLUCIÓN',
			'REAC_FECHA' => 'F. RESOLUCIÓN',
			'REAC_DESCRIPCION' => 'Reac Descripcion',
			
			'CONRATANTE' => 'RECTOR',
			'INICIO' => 'INICIO DE ENCARGO ',
			'FIN' => 'FIN DE ENCARGO',
		);
	}
	...
	$sql = "SELECT MAX(REAC_ID) FROM TBL_RESOLUCIONESACUERDOS";
	 $connection = Yii::app()->db;
	 //sleep(3600);
	 $query = $connection->createCommand($sql)->queryColumn();
	 $lastId = $query[0];
	 $model = Resolucionesacuerdos::model()->findByPk($lastId);
	 return $model;
	}
