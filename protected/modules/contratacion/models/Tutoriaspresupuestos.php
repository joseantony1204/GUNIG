 ...
 * @property string $TUPR_FECHA_INGRESO
 *
 * The followings are the available model relations:
 * @property TblPresupuestos $pRES
 */
class Tutoriaspresupuestos extends CActiveRecord
{
	/**
	...
	{
		return 'TBL_TUTORIASPRESUPUESTOS';
	}
	...
	public function rules()
	{
		...
		return array(
			array('PRES_ID, TUPR_FECHA_INGRESO', 'required'),
			array('PRES_ID', 'numerical', 'integerOnly'=>true),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('PRES_ID, TUPR_FECHA_INGRESO, PRES_NUM_CERTIFICADO, PRES_DESCRIPCION, 
			PRES_SECCION, PRES_CODIGO, PRES_MONTO, PRES_FECHA_VIGENCIA, PRES_NOMBRE, 
			PRES_FECHA_INGRESO, TOTALCONTRATADO', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'PRES_NOMBRE' => 'NOMBRE',
			'TUPR_FECHA_INGRESO' => 'FECHA RECIBIDO',
	...
	$sort->attributes = array(
			...
			'TOTALCONTRATADO'=>array(
				'asc'=>'(SELECT ROUND(SUM(tt.TUTO_VALOR)) FROM TBL_TUTORIAS tt WHERE tt.TUPR_ID = t.TUPR_ID)',
				'desc'=>'(SELECT ROUND(SUM(tt.TUTO_VALOR)) FROM TBL_TUTORIAS tt WHERE tt.TUPR_ID = t.TUPR_ID) desc',
			),
		...
		$criteria->select = 't.TUPR_ID, p.PRES_ID, p.PRES_NUM_CERTIFICADO, p.PRES_DESCRIPCION, p.PRES_SECCION, p.PRES_CODIGO, 
		p.PRES_MONTO, p.PRES_NOMBRE, t.TUPR_FECHA_INGRESO, p.PRES_FECHA_VIGENCIA,
		(SELECT ROUND(SUM(tt.TUTO_VALOR)) FROM TBL_TUTORIAS tt WHERE tt.TUPR_ID = t.TUPR_ID) AS TOTALCONTRATADO';
        
		$criteria->join = 'INNER JOIN TBL_PRESUPUESTOS p ON p.PRES_ID = t.PRES_ID';
		
		$criteria->compare('PRES_ID',$this->PRES_ID);
		$criteria->compare('TUPR_FECHA_INGRESO',$this->TUPR_FECHA_INGRESO,true);
