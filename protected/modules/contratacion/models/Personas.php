...
class Personas extends CActiveRecord
{
	 ...
	 */
	public $CONT_ID, $OPCO_ID, $OPCO_MESES, $OPCO_DIAS, $DEPE_NOMBRE, $OPCO_VALOR_MENSUAL, 
	$ANAC_NOMBRE, $CONT_FECHAINICIO, $CONT_FECHAFINAL, $NOMBRE;
	...
	public function rules()
	{
		...
		return array(
            ...
            array('PERS_IDENTIFICACION', 'unique'),
	...
	public function relations()
	{
		...
		return array(
			...
			'rel_personas_juridicas' => array(self::HAS_ONE, 'Personasjuridicas', 'PERS_ID'),
		);
	}
	...
	$sql = "SELECT MAX(PERS_ID) FROM TBL_PERSONAS 
  ...
  public function nombrePersonas()
	...
	public function nombrePersonasNaturales()
	{
     $connection = Yii::app()->db;
	 $sql = "SELECT t.PERS_ID, CONCAT(pn.PENA_NOMBRES, ' ' , pn.PENA_APELLIDOS) AS NOMBRE
	 FROM TBL_PERSONAS t
  ...
  public function obtenerMisContratos()
	...
	{
     $connection = Yii::app()->db;
	 $sql = "SELECT CONCAT(pn.PENA_APELLIDOS, ' ' , pn.PENA_NOMBRES) AS NOMBRE
	 FROM TBL_PERSONAS t
	 INNER JOIN TBL_PERSONASNATURALES pn ON pn.PERS_ID = t.PERS_ID AND t.PERS_ID = ".$id."
	 UNION
	 SELECT pj.PEJU_NOMBRE
	 FROM TBL_PERSONAS t
	 INNER JOIN TBL_PERSONASJURIDICAS pj ON pj.PERS_ID = t.PERS_ID AND t.PERS_ID = ".$id."";
	 $data = $connection->createCommand($sql)->queryColumn(); 
	 $nombre = $data[0];
	 return $nombre; 
	}
