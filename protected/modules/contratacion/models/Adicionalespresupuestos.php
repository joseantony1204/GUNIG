 ...
 */
class Adicionalespresupuestos extends CActiveRecord
{
	/**
	...
	{
		return 'TBL_ADICIONALESPRESUPUESTOS';
	}
	...
	public function rules()
	{
		...
		return array(
			array('PRES_ID, ADPR_FECHA_INGRESO', 'required'),
			array('PRES_ID', 'numerical', 'integerOnly'=>true),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('PRES_ID, ADPR_FECHA_INGRESO, PRES_NUM_CERTIFICADO, PRES_DESCRIPCION, 
	...
	public function attributeLabels()
	{
		return array(
			...
			'PRES_NOMBRE' => 'NOMBRE',
			'ADPR_FECHA_INGRESO' => 'FECHA RECIBIDO',
	...
	$sort->attributes = array(
			...
			'TOTALCONTRATADO'=>array(
				'asc'=>'(SELECT ROUND((SUM(((ca.COAD_VALOR)*(ca.COAD_MESES))+((ca.COAD_VALOR/30)*(ca.COAD_DIAS)))))
		FROM TBL_CONTRATOSADICIONALES ca WHERE t.ADPR_ID = ca.ADPR_ID AND ca.ADPR_ID = t.ADPR_ID)',
				'desc'=>'(SELECT ROUND((SUM(((ca.COAD_VALOR)*(ca.COAD_MESES))+((ca.COAD_VALOR/30)*(ca.COAD_DIAS)))))
		FROM TBL_CONTRATOSADICIONALES ca WHERE t.ADPR_ID = ca.ADPR_ID AND ca.ADPR_ID = t.ADPR_ID) desc',
			),
		...
		$criteria->select = 't.ADPR_ID, p.PRES_ID, p.PRES_NUM_CERTIFICADO, p.PRES_DESCRIPCION, p.PRES_SECCION, p.PRES_CODIGO, 
		p.PRES_MONTO, p.PRES_NOMBRE, t.ADPR_FECHA_INGRESO, p.PRES_FECHA_VIGENCIA,
	   (SELECT ROUND((SUM(((ca.COAD_VALOR)*(ca.COAD_MESES))+((ca.COAD_VALOR/30)*(ca.COAD_DIAS)))))
		FROM TBL_CONTRATOSADICIONALES ca WHERE t.ADPR_ID = ca.ADPR_ID AND ca.ADPR_ID = t.ADPR_ID) AS TOTALCONTRATADO';
        $criteria->join = 'INNER JOIN TBL_PRESUPUESTOS p ON p.PRES_ID = t.PRES_ID';
        
		
		$criteria->compare('PRES_ID',$this->PRES_ID);
		$criteria->compare('ADPR_FECHA_INGRESO',$this->ADPR_FECHA_INGRESO,true);
