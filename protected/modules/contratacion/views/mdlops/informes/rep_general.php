...
$objPHPExcel->setActiveSheetIndex($index)
								 ...
								 ->setCellValue('G8', 'DEPENDENCIA')
								 ->setCellValue('H8', 'CED. SUPERVISOR')
								 ->setCellValue('I8', 'NOMB. SUPERVISOR')
								 ->setCellValue('J8', 'APE. SUPERVISOR')
								 ->setCellValue('K8', 'MESES')
								 ->setCellValue('L8', 'DIAS') 
								 ->setCellValue('M8', 'VALOR MENSUAL')
								 ->setCellValue('N8', 'VALOR TOTAL')
								 ->setCellValue('O8', 'VALOR CON 4XMIL')
								 ->setCellValue('P8', 'OBJETO')
								 ->setCellValue('Q8', 'N. CERTIFICADO')
								 ->setCellValue('R8', 'F. VIGENCIA')
								 ->setCellValue('S8', 'DESCRIPCION')
								 ->setCellValue('T8', 'MONTO');
					
$fxls=10;
$item = 1;

$objPHPExcel->getActiveSheet()->getStyle('B1:B6')->applyFromArray($styleArray);
$objPHPExcel->getActiveSheet()->getStyle('B8:T8')->applyFromArray($styleArrayB);
...
foreach($registros as $data){
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('G'.$fxls, $data["DEPE_NOMBRE"]);      
 $objPHPExcel->getActiveSheet()->setCellValue('H'.$fxls, $Personasnaturales->rel_personas->PERS_IDENTIFICACION);
 $objPHPExcel->getActiveSheet()->setCellValue('I'.$fxls, $Personasnaturales->PENA_NOMBRES);
 $objPHPExcel->getActiveSheet()->setCellValue('J'.$fxls, $Personasnaturales->PENA_APELLIDOS);
 $objPHPExcel->getActiveSheet()->setCellValue('K'.$fxls, $data["OPCO_MESES"]);
 $objPHPExcel->getActiveSheet()->setCellValue('L'.$fxls, $data["OPCO_DIAS"]);
 $objPHPExcel->getActiveSheet()->setCellValue('M'.$fxls, $data["OPCO_VALOR_MENSUAL"]);
 $objPHPExcel->getActiveSheet()->setCellValue('N'.$fxls, $valorContrato);
 $objPHPExcel->getActiveSheet()->setCellValue('O'.$fxls, $valorContrato4xMil);
 $objPHPExcel->getActiveSheet()->setCellValue('P'.$fxls, $data["OBJE_NOMBRE"]);
 $objPHPExcel->getActiveSheet()->setCellValue('Q'.$fxls, $data["PRES_NUM_CERTIFICADO"]);
 $objPHPExcel->getActiveSheet()->setCellValue('R'.$fxls, $data["PRES_FECHA_VIGENCIA"]);
 $objPHPExcel->getActiveSheet()->setCellValue('S'.$fxls, $data["PRES_DESCRIPCION"]);
 $objPHPExcel->getActiveSheet()->setCellValue('T'.$fxls, $data["PRES_MONTO"]);
 ...
 $item++; 
  ...
  $objPHPExcel->getActiveSheet()->getStyle('T10:T'.$fxls)->getNumberFormat()->setFormatCode('#,##0');
