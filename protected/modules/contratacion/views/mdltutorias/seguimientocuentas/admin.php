...
<td width="10%" align="center"><?php
         ...
         echo CHtml::link($image, array('mdltutorias/cuentas/admin','id'=>$Cuentas->CONT_ID),$htmlOptions ); 
?></td>
...
<td width="10%" align="center"><?php
         ...
         $image = CHtml::image($imageUrl);
         echo CHtml::link($image, array('mdltutorias/seguimientocuentas/admin','id'=>$model->CUEN_ID),$htmlOptions ); 
?></td>
            </tr>
          </table>
          </fieldset>
    ...
    $Seguimientocuentas = Seguimientocuentas::model()->findByPk($seguimiento);
     if($Seguimientocuentas->SECU_ESTADO ==0){
	  ...
	  $detalles = "Cuenta tramitada con exito.";
	       ...
	       $icon = $DepIconosDev[$i];
		   $url = "mdltutorias/devolucionescuentas/admin/id/".$Seguimientocuentas->SECU_ID;
		   $detalles = "La cuenta ha tenido inconvenientes y a sido devuelta. Ir a Detalles?";
		  }
   }else{
	     $icon = $DepIconosDef[$i];
		 $url = "mdltutorias/seguimientocuentas/admin/id/".$model->CUEN_ID;
