		 ...
		 <tr>
		  <td>
					  ...
					  <tr>
    					...
    					<td width="1%" align="left">&nbsp;</td>
						<td width="47%" rowspan="2" align="left">
						<strong>'.$condescon.'</strong>						</td>
					  </tr>
					  <tr>
						<td width="52%" align="left"><br>
...
//	  $pdf->SetFont('times', 'B', '20', true);
  ...
  elseif(($Formclasescontratos->FCCO_ID)==5){/*SI ES UNA ORDEN DE CONSULTORIA CON POLIZA */
      $Formclasescontratos->FCCO_ID=201;
	  /* CARGANDO EL FORMATO DEL CONTRATO */  
	  $criteria = new CDbCriteria;
	  $criteria->condition = 'FCCO_ID = '.$Formclasescontratos->FCCO_ID;
	  $Formdescripcionclausulas = Formdescripcionclausulas::model()->findAll($criteria);
  ...
  elseif(($Formclasescontratos->FCCO_ID)==601){/*SI ES UNA ORDEN DE ARRENDAMIENTO CON POLIZA */
   ...
   elseif(($Formclasescontratos->FCCO_ID)==1802){/*SI ES UNA ORDEN PFAN  DE PRESTACION DE SERVICIOS SIN POLIZA */
     //echo $Formclasescontratos->FCCO_ID;
	  /* CARGANDO EL FORMATO DEL CONTRATO */  
	  $criteria = new CDbCriteria;
	  $criteria->condition = 'FCCO_ID = '.$Formclasescontratos->FCCO_ID;
...
//	  $pdf->SetFont('times', 'B', '20', true);
	   ...
	   <tr>
		...
		<tr>
		 ...
		 '; 
			 ...
			 $i=0;
			 $Cdp = Presupuestos::model()->findByPk($rows["PRES_ID"]);
			 $cdpnum[] = $Cdp->PRES_NUM_CERTIFICADO;
			 //$cdpvig[] = $Cdp->PRES_FECHA_VIGENCIA;
			 $cdpdia[] = date("d",strtotime( $Cdp->PRES_FECHA_VIGENCIA));
 		 	 $cdpmes[] = NombreMes(date("m",strtotime( $Cdp->PRES_FECHA_VIGENCIA)));
  		 	 $cdpani[] = date("Y",strtotime( $Cdp->PRES_FECHA_VIGENCIA)); 
  		     $cdpvig[] = $cdpdia[$i]." de ".$cdpmes[$i]." de ".$cdpani[$i];
			 $cdpcod[] = $Cdp->PRES_CODIGO;		
			 $descdp[]= "No. ".$cdpnum[$i]." del ".$cdpvig[$i].", código ".$cdpcod[$i]."";
			 $i ++;
			 }
				 if ($numerocdp==1) { 
					$html .='	
  ...
  elseif(($Formclasescontratos->FCCO_ID)==1801){/*SI ES UNA ORDEN   PFAN    DE PRESTACION DE SERVICIOS CON POLIZA */
      /* CARGANDO EL FORMATO DEL CONTRATO */  
	  $criteria = new CDbCriteria;
	  $criteria->condition = 'FCCO_ID = '.$Formclasescontratos->FCCO_ID;
	  $Formdescripcionclausulas = Formdescripcionclausulas::model()->findAll($criteria);
  ...
  elseif(($Formclasescontratos->FCCO_ID)==801){/*SI ES UN CONTRATO DE PRESTACION DE SERVICIOS GENERALES*/
	...
	$html .='
	 ...
	 $pdf->SetFont('times', 'K', 10.5);
     $pdf->writeHTML($html, true, 0, true, 0);
	   
  } 
  
  
    elseif(($Formclasescontratos->FCCO_ID)==803){/*SI ES UN CONTRATO DE PRESTACION DE SERVICIOS DE ARRENDAMIENTO DE VEHICULOS*/
      /* CARGANDO EL FORMATO DEL CONTRATO */  
	  $criteria = new CDbCriteria;
	  $criteria->condition = 'FCCO_ID = '.$Formclasescontratos->FCCO_ID;
	  $Formdescripcionclausulas = Formdescripcionclausulas::model()->findAll($criteria);
   ...
   elseif(($Formclasescontratos->FCCO_ID)==1401){/*SI ES UN CONTRATO DE OBRA*/
	...
	$html .=' '.$partesDescripcionClausula[12][0].' <strong>'.$objeto.'.</strong> './*$partesDescripcionClausula[13][0]*/$N=''.'
		...
		'.$partesDescripcionClausula[19][0].' <strong>'.$valorContratoLetras.' 
		($'.number_format($valorContrato).') M/CTE. </strong>'.$partesDescripcionClausula[20][0].'. '.$partesDescripcionClausula[21][0].'.
   ...
   $pdf->writeHTML($html, true, 0, true, 0);  
  ...
  }  elseif(($Formclasescontratos->FCCO_ID)==1701){/*SI ES UN CONTRATO DE PRESTACION DE SERVICIOS CON RUBRO DE GARANTIAS*/
      /* CARGANDO EL FORMATO DEL CONTRATO */  
	  $criteria = new CDbCriteria;
	  $criteria->condition = 'FCCO_ID = '.$Formclasescontratos->FCCO_ID;
	  $Formdescripcionclausulas = Formdescripcionclausulas::model()->findAll($criteria);
	  ...
	  ///***** CREANDO ARCHIVO *****///
	   ...
	   <tr>
		 <td align="center"><strong>'.$tipo.' - '.$numerocontrato.' DE PRESTACIÓN DE SERVICIOS DE '.$anio_contrato.'</strong></td>
	   </tr>
	   <tr>
		 <td align="center">&nbsp;</td>
	   </tr>
	...
	$html .='
		...
		<strong><u>CLÁUSULA '.$nombreClausulas[15].' - '.$descripcionClausula[15].'</u></strong>: '.$partesDescripcionClausula[20][0].''.$presupu.'
		 ...
		 '; 
			 ...
			 $descdp[]= "No. ".$cdpnum[$i]." del ".$cdpvig[$i].", código ".$cdpcod[$i]."";
			 $i ++;
			 //$descdp[]= "No. ".$cdpnum[$i]." expedido con fecha ".$cdpvig[$i]." y código ".$cdpcod[$i]."";
			 }
				 if ($numerocdp==1) { 
					$html .='	
					 <strong> '.$descdp[0].'.</strong> 
	...
	$html .='
     ...
     $pdf->writeHTML($html, true, 0, true, 0);
  ...
  } elseif(($Formclasescontratos->FCCO_ID)==1101){/*SI ES UN CONTRATO DE CONSULTORIA*/
      /* CARGANDO EL FORMATO DEL CONTRATO */  
	  $criteria = new CDbCriteria;
	  $criteria->condition = 'FCCO_ID = '.$Formclasescontratos->FCCO_ID;
	  $Formdescripcionclausulas = Formdescripcionclausulas::model()->findAll($criteria);
	  ...
	  ///***** CREANDO ARCHIVO *****///
	   ...
	   <tr>
		 <td align="center"><strong>'.$tipo.' - '.$numerocontrato.' DE CONSULTORÍA DE '.$anio_contrato.'</strong></td>
	   </tr>
	   <tr>
		 <td align="center">&nbsp;</td>
	   </tr>
