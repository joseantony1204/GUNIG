	...
	public function actionChangeState($id, $estado){
		...
		if($estado==1){
			...
			if ($Contratos->CLCO_ID == 140){
				$this->redirect(array('segcuenta/liquidaciones/createcobra','id'=>$id));
			} else{
		...
		if($estado>=3){
			...
			if ($Contratos->CLCO_ID == 140){
				$this->redirect(array('segcuenta/liquidaciones/updatecobra','id'=>$dato->LIQU_ID));
			} else{
		...
		if($estado==2){
			...
			$this->redirect(array('segcuenta/seguimientocuentas/admin','id'=>$model->CUEN_ID));						
		}		
	}	
   ...
   public function actiongenerarOrden($id, $estado){
		...
		if($estado>=3 & $dato != ''){
			$this->redirect(array('segcuenta/liquidaciones/download','id'=>$dato->LIQU_ID));			
					 ...
					 Yii::app()->user->setFlash('error','Lo sentimos, la cuenta debe ser liquidada para poder imprimir la Orden.');
					 $this->redirect(array('segcuenta/cuentas/admin','id'=>$model->CONT_ID));
	...
	public function actionSearch()
	{
		...
		{
			$model->attributes=$_POST['Cuentas'];
			$model->CUEN_ESTADO=$_POST['Cuentas']['CUEN_ESTADO'];
						
			$this->redirect(array('segcuenta/cuentas/Excel','inicial'=>$model->CUEN_FECHAINICIO,'final'=>$model->CUEN_FECHAFINAL,
			'estado'=>$model->CUEN_ESTADO, 'dependencia'=>$Seguimientouserdependencias->DEPE_ID));	
		}
	...
	public function actionExcel($inicial, $final, $estado, $dependencia) 
    {
		$model = Cuentas::model()->consultarLiquidaciones($inicial, $final, $estado, $dependencia);
		
		Yii::app()->request->sendFile('listado.xls',$this->renderPartial('viewExcel',array('model'=>$model,),true));
	}
