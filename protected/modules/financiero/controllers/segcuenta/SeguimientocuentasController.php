...
class SeguimientocuentasController extends Controller
{
	...
	public function actionCreate($id)
	{
		...
		if((isset($_POST['Seguimientocuentas'])) && (isset($_POST['Cuentas'])))
		{
			...
			$Seguimiento = $model->Seguimiento($id, $Seguimientouserdependencias->DEPE_ID);
			...
			$elimina = $model->eliminaSeguimiento($id, $Seguimientouserdependencias->DEPE_ID);
			...
			if($model->save()){
				...
				if($model->SECU_ESTADO==1){
				 ...
				 $this->redirect(array('segcuenta/devolucionescuentas/create','id'=>$model->SECU_ID));
					  ...
					  $mail->Username = 'joseantony1204@gmail.com'; 					  
					  $mail->Port = '465'; 
					  $mail->Password = '1204872506'; 
					  $mail->SMTPAuth   = true;
					    
					  $mail->From = "joseantony1204@gmail.com";
                      $mail->FromName = "Universidad de la Guajira - Seguimiento De Cuentas";
		 
					  $mail->SetFrom('joseantony1204@gmail.com', 'JOSE ANTONIO'); 
					  $mail->Subject = 'PHPMailer Test Subject via GMail, basic with authentication'; 
					  $mail->AltBody = 'To view the message, please use an HTML compatible email viewer!'; 
					  $mail->MsgHTML('<h1>JUST A TEST!</h1>'); 
					  $mail->AddAddress('ajgonzalez@uniguajira.edu.co', 'John Doe'); 
					  if(!$mail->Send()) {
                       Yii::app()->user->setFlash('success','Notificación enviada al correo del contratante...');
					  }else{
							Yii::app()->user->setFlash('error','Error enviando notificación al correo del contratante...');
						   }
					*/	   					 
 ...
 $Usuarios = Usuarios::model()->findByPk(Yii::app()->user->id);
 $Usuariosdependencias = Seguimientouserdependencias::model()->findBySql("SELECT * FROM TBL_SEGUIMIENTOUSERDEPENDENCIAS WHERE USUA_ID = ".$Usuarios->USUA_ID);
		$Seguimientouserdependencias = Seguimientouserdependencias::model()->findByPk($Usuariosdependencias->SEUD_ID);
		if($Seguimientouserdependencias->DEPE_ID==1 OR $Seguimientouserdependencias->DEPE_ID==14){
			$estadoCta = 0; 
			}elseif($Seguimientouserdependencias->DEPE_ID==49){
