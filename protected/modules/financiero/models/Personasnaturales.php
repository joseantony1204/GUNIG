...
class Personasnaturales extends CActiveRecord
{
	...
	public function search()
	{
		...
		t.MUNI_ID, t.SEXO_ID ';
	    $criteria->join = '
	    INNER JOIN TBL_PERSONAS p ON t.PERS_ID = p.PERS_ID AND t.PENA_ID = '.$Usuario->PENA_ID; 
    ...
    public function getNombreCompletoId($id){
		...
		$criteria->order= 'PERS_ID ASC';
		$personanatural = Personasnaturales::model()->findAll($criteria);		 
		$nombre=null;
		foreach($personanatural as $pena){ 
		 $nombre=$pena->PENA_NOMBRES .' '. $pena->PENA_APELLIDOS; 
		}
		return $nombre;
	}
