...
class AcreditacionsoportesController extends Controller
{
	/**
	...
	{
		$model=new acreditacionsoportes;
		$model_acredi=new acreditaciones;
		if(isset($_GET['INDICADOR'])){ $model->ACIN_ID = $_GET['INDICADOR'];  }

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['acreditacionsoportes']))
		{
			$model->attributes=$_POST['acreditacionsoportes'];
			if($model->save()){ $this->numerar_grupos();
							$this->redirect(array('view','id'=>$model->ACSO_ID));
			}
		}
	
	$model->ACSO_NUMERO = acreditacionsoportes::model()->count()+1;		
	...
	{	$criterio = array('order'=>'ACIN_ID ASC');
		$modelos=acreditacionsoportes::model()->findAll($criterio);
		$k=1;//count(*)
		$var='';
		foreach($modelos as $model){
			if($var==$model['ACIN_ID']){
				$k = $k + 1;
			}else{
				$k=1;
			}
			$sql = 'UPDATE TBL_ACREDITACIONSOPORTES SET ACSO_NUMERO = '.$k.' WHERE ACSO_ID = '.$model['ACSO_ID'];	
	...
	{
		$dataProvider=new CActiveDataProvider('acreditacionsoportes');
	...
	{
		$model=new acreditacionsoportes('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['acreditacionsoportes']))
			$model->attributes=$_GET['acreditacionsoportes'];
	...
	{
		$model=acreditacionsoportes::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='acreditacionsoportes-form')
		{
