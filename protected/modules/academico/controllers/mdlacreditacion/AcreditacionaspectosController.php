...
class AcreditacionaspectosController extends Controller
{
	/**
	...
	{
		$model=new acreditacionaspectos;
		$model_acredi=new acreditaciones;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);
		if(isset($_GET['CARACTERISTICA'])){ $model->ACCA_ID = $_GET['CARACTERISTICA'];  }
		
		if(isset($_POST['acreditacionaspectos']))
		{
			$model->attributes=$_POST['acreditacionaspectos'];
			if($model->save()){$this->numerar_grupos();			
				$this->redirect(array('view','id'=>$model->ACAS_ID));
			}
		}
		
		$model->ACAS_NUMERO = acreditacionaspectos::model()->count()+1;		
	...
	{	$criterio = array('order'=>'ACCA_ID ASC');
		$modelos=acreditacionaspectos::model()->findAll($criterio);
		$k=1;//count(*)
		$var='';
		foreach($modelos as $model){
			if($var==$model['ACCA_ID']){
				$k = $k + 1;
			}else{
				$k=1;
			}
			$sql = 'UPDATE TBL_ACREDITACIONASPECTOS SET ACAS_NUMERO = '.$k.' WHERE ACAS_ID = '.$model['ACAS_ID'];	
	...
	{
		$dataProvider=new CActiveDataProvider('acreditacionaspectos');
	...
	{
		$model=new acreditacionaspectos('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['acreditacionaspectos']))
			$model->attributes=$_GET['acreditacionaspectos'];
	...
	{
		$model=acreditacionaspectos::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='acreditacionaspectos-form')
		{
